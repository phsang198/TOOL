...
class api:
    ...
    def __init__(self, name, ownerid, secret, version, hash_to_check):
        if len(ownerid) != 10 and len(secret) != 64:
            raise Exception("Applications Error")
    ...
    def license(self, key, hwid=None):
        ...
        if json["success"]:
            self.__load_user_data(json["info"])
            return True,"successfully logged into license"
        else:
...
class others:
    ...
    def get_hwid():
        if platform.system() == "Linux":
            ...
            cmd = subprocess.Popen("wmic cpu get processorid",stdout=subprocess.PIPE,shell=True,creationflags=subprocess.CREATE_NO_WINDOW)
